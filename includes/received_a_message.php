<?php
if (isset($_POST['received_message1']) && ((int) $_POST['sensor_id1']>0 || (int) $_POST['user_id']>0 )) {
...
$file = fopen(INCLUDES_PATH."post.log", "w") or die("Unable to open file!");
fwrite($file, $post);
fclose($file);
$i=1;
while (isset($_POST['sensor_id'.$i])){
    $need_to_insert=true;
            ...
            if ((int) $_POST['message_type'.$i]<3) //on error or warning we should notify somebody
                ...
                $SQL="INSERT INTO telegram_messages (user_id,sensor_id,received_message,sensor_value) VALUES (".$user.",".(int) $_POST['sensor_id'.$i].",".(int) $_POST["received_message".$i].",".(float)$_POST['sensor_value'.$i].")";
                $dba->Query($SQL);
                if (LM_DEBUG)
                error_log($SQL,0); 
                }
