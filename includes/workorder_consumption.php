...
$SQL="select ROUND(SUM(TIME_TO_SEC(workorder_worktime)/3600),1) as workhour, username FROM workorder_works LEFT JOIN users ON workorder_works.workorder_user_id=users.user_id WHERE workorder_works.workorder_id=".$workorder_id." GROUP BY workorder_user_id ORDER BY workhour DESC";
...
$SQL="SELECT workorder_id, workorder_work_id,workorder_work_start_time,workorder_work_end_time,workorder_work,workorder_user_id,workorder_partner_id FROM workorder_works WHERE workorder_id='". $workorder_id."'";
